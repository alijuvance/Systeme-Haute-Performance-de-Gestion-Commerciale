...
export function PurchasesTable() {
  ...
  const columns: ColumnDef<Purchase>[] = [
    ...
    },
    {
      ...
      cell: (p) => {
        const variant = p.status === 'RECEIVED' ? 'success' : p.status === 'SENT' ? 'info' : 'default';
        const icon = p.status === 'RECEIVED' ? <PackageCheck className="w-3 h-3"/> : p.status === 'SENT' ? <Truck className="w-3 h-3"/> : undefined;
        const text = p.status === 'RECEIVED' ? 'Reçu' : p.status === 'SENT' ? 'En transit' : p.status === 'DRAFT' ? 'Brouillon' : p.status;
        ...
        {selectedOrder && (
          <div className="space-y-5">
            ...
            <div className="flex justify-end gap-3 pt-5 mt-6 border-t border-gray-100">
              ...
              <Button onClick={handleReceive} isLoading={isSubmitting} disabled={!depotIdInput}>Réceptionner</Button>
            </div>
          </div>
        )}
      </Modal>
